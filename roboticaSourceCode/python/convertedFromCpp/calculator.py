...
length_ulna = 32.0  # Length of the ulna in units (cm) 32
...
def calculate_camera_coords():
	base_rotation_angle = -getPosition(id_base) / 3.33 * (math.pi/180) # Angle between base and humerus in rad
	elbow_rotation_angle = getPosition(id_elbow) / 2 * (math.pi/180) # Angle between humerus and ulna in rad
	...
	print(elbow_theta*180/math.pi, elbow_rotation_angle*180/math.pi,base_rotation_angle*180/math.pi)
...
def calculate_real_life_coordinates(screen_x, screen_y):
	...
	print(f"\nCamera coordinates: ({camera_coords.x}, {camera_coords.y})\n")
	# print(f"Elbow theta (in radians): {elbow_theta}")
	# print(f"Elbow theta (in degrees): {elbow_theta * (180 / math.pi)}")

	conversion_rate = 35/640 #pixels per cm
	...
	#print(f"thetas: {elbow_theta*180/math.pi} {theta_centroid*180/math.pi} {theta_centroid_real_life*180/math.pi}")

	targetx = camera_coords.x + (distance_camera_centroid*math.cos(theta_centroid))
	targety = camera_coords.y + (distance_camera_centroid*math.sin(theta_centroid))


	target_coords = Point(targetx,targety)
	#target_coords = Point(60,-10)
	return [target_coords, elbow_theta]
...
def calculate_distance_to_target(x, y):
	...
	print(distance_from_base_to_target)
...
def calculate_gripper_angle(vx, vy, baseangle,elbowangle):
	# Perpendicular direction vector
	px = -vy
	py = vx
	req_elbow_theta = -baseangle + elbowangle
	# Calculate the angle with respect to the x-axis
	angle_radians = math.atan2(py, px)
	angle_degrees = angle_radians * (180.0 / math.pi) + req_elbow_theta 
	print("req: ",req_elbow_theta)
...
def calculate_arm_angles(x, y, distance_to_target):  # x and y of the center of mass of the object, along with the distance
	...
	arm_angles[1] = (math.acos((distance_to_target**2 + length_humerus**2 - length_ulna**2) / (2 * length_humerus * distance_to_target)) + math.atan2(y, x)) * (180 / math.pi)  # Base angle
	return arm_angles
...
def calculator(x, y, vx, vy):
	...
	calcreal = calculate_real_life_coordinates(x, y)  # The point where gripper is going to be heading
	pointC = calcreal[0]
	elbow_theta = calcreal[1]
	#pointC = calculate_real_life_coordinates(320, 240)  # The point where gripper is going to be heading
	print(f"relative coords {x} {y}")
	print(f"absolute coords target {round(pointC.x,1)} {round(pointC.y,1)}\n")
	...
	results[0] = calculate_gripper_angle(vx, vy,arm_angles[1],arm_angles[0])  # Calculate angle for the gripper to be perpendicular to the scissors
