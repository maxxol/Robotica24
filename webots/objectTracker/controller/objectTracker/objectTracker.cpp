...
// Get the node of the box using its DEF name

Supervisor *robot = new Supervisor();

// Get the node of the box using its DEF name
webots::Node *boxNode = robot->getFromDef("MY_BOX");
webots::Field *translationField = boxNode->getField("translation");
...
void calculateRequiredArmAngles(double coordinateX, double coordinateY, double distanceFromBaseToTarget){
...
std::pair<bool, double> canReachCoordinates(double coordinateX, double coordinateY){
    double distanceFromBaseToTarget = sqrt(pow(coordinateX, 2) + pow(coordinateY, 2));

    // Check if the target is reachable for the gripper
   bool isReachable = !(distanceFromBaseToTarget > armBone1Length + armBone2Length || distanceFromBaseToTarget < abs(armBone1Length - armBone2Length));
    ...
    return std::make_pair(isReachable, distanceFromBaseToTarget);
}

bool arrivedAtCoordinates(double coordinateX, double coordinateY){
    if (arm1MotorAngle < requiredArm1MotorAngle+0.02 && arm1MotorAngle > requiredArm1MotorAngle-0.02) {
        arm1Motor->setVelocity(0);
        arm1HasReachedPosition = true;
        //std::cout << "Arm 1 reached target angle" << std::endl;
    }
    
    if(arm2MotorAngle < requiredArm2MotorAngle+0.02 && arm2MotorAngle > requiredArm2MotorAngle-0.02) {
        arm2Motor->setVelocity(0);
    ...
    std::cout << "not at coords" << std::endl;
    return false;
    	}
...
int main() {
  ...
  double speed = 2; // it go fast or it go slow
  ...
  // Check if the box node was found
  if (!boxNode) {
    std::cerr << "No DEF MY_BOX node found in the world file." << std::endl;
  ...
  while (robot->step(64) != -1) {// This loop will continue running until the simulation is stopped
   ...
   const double *boxPosition = translationField->getSFVec3f();
   std::cout << "Box position: x=" << boxPosition[0] << " y=" << boxPosition[1] << " z=" << boxPosition[2] << std::endl;
   //const double *boxPosition = boxNode->getPosition();
   double x = boxPosition[0];
   double y = boxPosition[1];
   
   std::pair<bool, double> canReachAndDistance = canReachCoordinates(x, y);
   calculateRequiredArmAngles(x, y, canReachAndDistance.second);

   
   if(!arrivedAtCoordinates(x,y) && canReachAndDistance.first){
   MoveArmToCoordinates(x,y,speed);
   }
